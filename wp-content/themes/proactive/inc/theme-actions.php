 ...
 * @package Case-Themes
 */
add_action('after_setup_theme', 'proactive_setup');
function proactive_setup(){

    //Set the content width in pixels, based on the theme's design and stylesheet.
    $GLOBALS['content_width'] = apply_filters( 'proactive_content_width', 1200 );

    // Make theme available for translation.
    load_theme_textdomain( 'proactive', get_template_directory() . '/languages' );
    ...
    register_nav_menus( array(
        'primary' => esc_html__( 'Primary Desktop', 'proactive' ),
        'primary-mobile' => esc_html__( 'Primary Mobile', 'proactive' ),
    ) );
    ...
    // Enable support for Post Thumbnails on posts and pages.
    add_theme_support('post-thumbnails');
    add_image_size( 'proactive-portfolio', 600, 600, true );
 ...
 */
add_action( 'widgets_init', 'proactive_widgets_position' );
function proactive_widgets_position() {
	register_sidebar( array(
		'name'          => esc_html__( 'Blog Sidebar', 'proactive' ),
		'id'            => 'sidebar-blog',
	...
	if (class_exists('ReduxFramework')) {
		register_sidebar( array(
			'name'          => esc_html__( 'Page Sidebar', 'proactive' ),
			'id'            => 'sidebar-page',
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title"><span>',
	...
	if ( class_exists( 'Woocommerce' ) ) {
		register_sidebar( array(
			'name'          => esc_html__( 'Shop Sidebar', 'proactive' ),
			'id'            => 'sidebar-shop',
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title"><span>',
 ...
 */
add_action( 'wp_enqueue_scripts', 'proactive_scripts' );
function proactive_scripts() {  
    $proactive_version = wp_get_theme( get_template() );
    ...
    /* Icons Lib - CSS */
    wp_enqueue_style('flaticon', get_template_directory_uri() . '/assets/fonts/flaticon/css/flaticon.css' , array(), $proactive_version->get( 'Version' ));
    ...
    /* Parallax Scroll */
    wp_enqueue_script( 'pxl-parallax-background', get_template_directory_uri() . '/assets/js/libs/parallax-background.js', array( 'jquery' ), $proactive_version->get( 'Version' ), true );
    wp_enqueue_script( 'pxl-parallax-scroll', get_template_directory_uri() . '/assets/js/libs/parallax-scroll.js', array( 'jquery' ), $proactive_version->get( 'Version' ), true );
    ...
    /* Woocommerce */
    wp_enqueue_script( 'pxl-woocommerce', get_template_directory_uri() . '/woocommerce/js/woocommerce.js', array( 'jquery' ), $proactive_version->get( 'Version' ), true );
	...
	wp_enqueue_style( 'pxl-caseicon', get_template_directory_uri() . '/assets/css/caseicon.css', array(), $proactive_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-grid', get_template_directory_uri() . '/assets/css/grid.css', array(), $proactive_version->get( 'Version' ) );
	wp_enqueue_style( 'pxl-style', get_template_directory_uri() . '/assets/css/style.css', array(), $r );
	wp_add_inline_style( 'pxl-style', proactive_inline_styles() );
    wp_enqueue_style( 'pxl-base', get_template_directory_uri() . '/style.css', array(), $proactive_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-google-fonts', proactive_fonts_url(), array(), null );
	wp_enqueue_script( 'pxl-main', get_template_directory_uri() . '/assets/js/theme.js', array( 'jquery' ), $proactive_version->get( 'Version' ), true );
    wp_localize_script( 'pxl-main', 'main_data', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
    if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
        wp_enqueue_script( 'comment-reply' );
    }
    do_action( 'proactive_scripts');
}

/**
 * Enqueue Styles Scripts : Back-End
 */
add_action('admin_enqueue_scripts', 'proactive_admin_style');
function proactive_admin_style() {
    $theme = wp_get_theme( get_template() );
    wp_enqueue_style( 'proactive-admin-style', get_template_directory_uri() . '/assets/css/admin.css', array(), $theme->get( 'Version' ) );
...
add_action( 'elementor/editor/before_enqueue_scripts', function() {
    ...
    wp_enqueue_style( 'proactive-admin-style', get_template_directory_uri() . '/assets/css/admin.css');
} );
 ...
 */
add_action( 'wp_head', 'proactive_pingback_header' );
function proactive_pingback_header(){
...
/* Hidden Panel */
add_action( 'pxl_anchor_target', 'proactive_hook_anchor_templates_hidden_panel');
function proactive_hook_anchor_templates_hidden_panel(){

    $hidden_templates = proactive_get_templates_slug('hidden-panel');
...
if(!function_exists('proactive_hook_anchor_hidden_panel')){
    function proactive_hook_anchor_hidden_panel($args){
...
/* Elementor Popup */
add_action( 'pxl_anchor_target', 'proactive_hook_anchor_templates_popup');
function proactive_hook_anchor_templates_popup(){

    $popup_templates = proactive_get_templates_slug('popup');
...
if(!function_exists('proactive_hook_anchor_popup')){
    function proactive_hook_anchor_popup($args){ ?>
...
/* Page Popup */
add_action( 'pxl_anchor_target', 'proactive_hook_anchor_templates_page_popup');
function proactive_hook_anchor_templates_page_popup(){

    $page_templates = proactive_get_templates_slug('page');
...
if(!function_exists('proactive_hook_anchor_page_popup')){
    function proactive_hook_anchor_page_popup($args){ ?>
...
/* Cart Sidebar */
if(!function_exists('proactive_hook_anchor_cart')){
    function proactive_hook_anchor_cart(){
        if(class_exists('Woocommerce')) :
            global $woocommerce; ?>
...
add_action('wp_ajax_nopriv_item_added', 'proactive_addedtocart_sweet_message');
add_action('wp_ajax_item_added', 'proactive_addedtocart_sweet_message');
function proactive_addedtocart_sweet_message() {
    echo isset($_POST['id']) && $_POST['id'] > 0 ? (int) esc_attr($_POST['id']) : false;
    die();
}
add_action('wp_footer', 'proactive_cart_hidden_sidebar');
function proactive_cart_hidden_sidebar() {
