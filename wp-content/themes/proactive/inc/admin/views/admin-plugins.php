...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'proactive');
...
$merlin_setup = get_option( 'merlin_' . proactive()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'proactive');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'proactive');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'proactive');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'proactive' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'proactive' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'proactive' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Proactive_Admin_Plugins;
