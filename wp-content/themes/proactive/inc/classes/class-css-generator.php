...
class Proactive_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = proactive()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'proactive_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'proactive_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->proactive_generate_file_options();
		} );
	}

	function proactive_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->proactive_generate_file_options();
			$this->proactive_generate_file();
		}
	}

    function proactive_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->proactive_options_output() )
        ) );
    }

	function proactive_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function proactive_options_output() {
		$theme_colors                    = proactive_configs('theme_colors');
        $links                           = proactive_configs('link');
        $gradients                       = proactive_configs('gradient');
        $gradients2                      = proactive_configs('gradient2');
		ob_start();

		printf('$proactive_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$proactive_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
        ...
        /* Font */
        $font_secondary = proactive()->get_theme_opt('f_secondary');
    ...
    /* Inline CSS */
    function proactive_enqueue() {
        $css = $this->proactive_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function proactive_inline_css() {
        ob_start();
