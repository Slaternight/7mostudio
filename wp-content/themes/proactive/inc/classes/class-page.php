...
if (!class_exists('Proactive_Page')) {

    class Proactive_Page
    {
        public function get_site_loader(){

            $site_loader = proactive()->get_theme_opt( 'site_loader', 'off' );
            $site_loader_style = proactive()->get_theme_opt( 'site_loader_style', 'style-1' );
            $site_loader_icon = proactive()->get_theme_opt( 'site_loader_icon' );
            if($site_loader == 'on') { ?>
        ...
        public function get_page_title(){
            ...
            $pt_mode = proactive()->get_opt('pt_mode');
            $pt_mode_product = proactive()->get_opt('pt_mode_product');
            $ptitle_scroll_opacity = proactive()->get_opt('ptitle_scroll_opacity');
            $ptitle_layout = (int)proactive()->get_opt('ptitle_layout');
            $ptitle_layout_product = (int)proactive()->get_opt('ptitle_layout_product');
            ...
            <?php } elseif($pt_mode == 'df') {
                $ptitle_breadcrumb_on = proactive()->get_opt( 'ptitle_breadcrumb_on', '1' ); 
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <h1 class="pxl-page-title"><?php echo proactive_html($titles['title']) ?></h1>
                            </div>
        ...
        public function get_title() {
            ...
            if ( ! is_archive() ) {
                ...
                elseif ( is_page() ) {
                    ...
                    $title = esc_html__( '404 Error', 'proactive' );
                } elseif ( is_search() ) {
                    $title = esc_html__( 'Search results', 'proactive' );
                } elseif ( is_singular('lp_course') ) {
                    $title = esc_html__( 'Course', 'proactive' );
                } else {
                    $title = get_post_meta( get_the_ID(), 'custom_title', true );
                    if ( ! $title ) {
                        $title = get_the_title();
        ...
        public function get_breadcrumb(){
            ...
            foreach ( $entries as $entry )
            {
                ...
                {
                    $sg_post_title = proactive()->get_theme_opt('sg_post_title', 'default');
                    $sg_post_title_text = proactive()->get_theme_opt('sg_post_title_text');
                    if(is_singular('post') && $sg_post_title == 'custom_text' && !empty($sg_post_title_text)) {
                        $entry_label = $sg_post_title_text;
                    }
                    $sg_product_ptitle = proactive()->get_theme_opt('sg_product_ptitle', 'default');
                    $sg_product_ptitle_text = proactive()->get_theme_opt('sg_product_ptitle_text');
        ...
        public function get_pagination( $query = null, $ajax = false ){
            ...
            if($ajax){
                add_filter('paginate_links', 'proactive_ajax_paginate_links');
            }
