...
if (!class_exists('Proactive_Blog')) {

    class Proactive_Blog
    {
        
        public function get_archive_meta() {
            $archive_author = proactive()->get_theme_opt( 'archive_author', true );
            $archive_date = proactive()->get_theme_opt( 'archive_date', true );
            $archive_category = proactive()->get_theme_opt( 'archive_category', true );
            $archive_comment = proactive()->get_theme_opt( 'archive_comment', true );
                    ...
                    <div class="pxl-blog-meta-inner">
                        <?php if($archive_author) : ?>
                            <div class="pxl-item--author pxl-mr-28">
                                ...
                                <?php echo esc_html__('by', 'proactive'); ?>&nbsp;<?php the_author_posts_link(); ?>
                            </div>
                        <?php endif; ?>
                        <?php if($archive_comment) : ?>
                            <div class="pxl-item--comment pxl-mr-28">
                                ...
                                <a href="<?php the_permalink(); ?>#comments">
                                    <?php echo comments_number(esc_html__('No Comments', 'proactive'),esc_html__('1 Comment', 'proactive'),esc_html__('% Comments', 'proactive')); ?>
                                </a>
                            </div>
                        <?php endif; ?>
                        <?php if($archive_date) : ?>
        ...
        public function get_post_metas(){
            $post_author = proactive()->get_theme_opt( 'post_author', true );
            $post_comment = proactive()->get_theme_opt( 'post_comment', true );
            $post_date = proactive()->get_theme_opt( 'post_date', true );
            $post_category = proactive()->get_theme_opt( 'post_category', true );
        ...
        public function get_excerpt(){
            $archive_excerpt_length = proactive()->get_theme_opt('archive_excerpt_length', '50');
            $proactive_the_excerpt = get_the_excerpt();
            if(!empty($proactive_the_excerpt)) {
                echo wp_trim_words( $proactive_the_excerpt, $archive_excerpt_length, $more = null );
            } else {
        ...
        public function get_excerpt_more( $post = null ) {
            ...
            if ( post_password_required( $post ) ) {
                return esc_html__( 'Post password required.', 'proactive' );
            }

            $content = apply_filters( 'the_content', strip_shortcodes( $post->post_content ) );
            $content = str_replace( ']]>', ']]&gt;', $content );

            $excerpt_more = apply_filters( 'proactive_excerpt_more', '&hellip;' );
            $excerpt      = wp_trim_words( $content, $archive_excerpt_length, $excerpt_more );

            return $excerpt;
        }

        public function proactive_set_post_views( $postID ) {
        ...
        public function get_tagged_in(){
            $tags_list = get_the_tag_list( '<label class="label">'.esc_attr__('Tags:', 'proactive'). '</label>', ' ' );
            if ( $tags_list )
            {
        ...
        public function get_socials_share() { 
            ...
            $social_facebook = proactive()->get_theme_opt( 'social_facebook', true );
            $social_twitter = proactive()->get_theme_opt( 'social_twitter', true );
            $social_pinterest = proactive()->get_theme_opt( 'social_pinterest', true );
            $social_linkedin = proactive()->get_theme_opt( 'social_linkedin', true );
            ?>
            <div class="pxl--social">
                <label><?php echo esc_html__('Share:', 'proactive'); ?></label>
                <?php if($social_facebook) : ?>
                    <a class="fb-social" title="<?php echo esc_attr__('Facebook', 'proactive'); ?>" target="_blank" href="http://www.facebook.com/sharer/sharer.php?u=<?php the_permalink(); ?>"><i class="caseicon-facebook"></i></a>
                <?php endif; ?>
                <?php if($social_twitter) : ?>
                    <a class="tw-social" title="<?php echo esc_attr__('Twitter', 'proactive'); ?>" target="_blank" href="https://twitter.com/intent/tweet?url=<?php the_permalink(); ?>&text=<?php the_title(); ?>%20"><i class="caseicon-twitter"></i></a>
                <?php endif; ?>
                <?php if($social_pinterest) : ?>
                    <a class="pin-social" title="<?php echo esc_attr__('Pinterest', 'proactive'); ?>" target="_blank" href="http://pinterest.com/pin/create/button/?url=<?php the_permalink(); ?>&media=<?php echo esc_url($img_url[0]); ?>&description=<?php the_title(); ?>%20"><i class="caseicon-pinterest"></i></a>
                <?php endif; ?>
                <?php if($social_linkedin) : ?>
                    <a class="lin-social" title="<?php echo esc_attr__('LinkedIn', 'proactive'); ?>" target="_blank" href="http://www.linkedin.com/shareArticle?mini=true&url=<?php the_permalink(); ?>&title=<?php the_title(); ?>%20"><i class="caseicon-linkedin"></i></a>
                <?php endif; ?>
            </div>
            <?php
        }
        ...
        public function get_post_author_info() { ?>
            <div class="pxl-post--author-info pxl-item--flexnw">
                ...
                <div class="pxl-post--author-meta">
                    <?php proactive_get_user_name(); ?>
                    <div class="pxl-post--author-description"><?php the_author_meta( 'description' ); ?></div>
                    <?php proactive_get_user_social(); ?>
                </div>
            </div>
        <?php }

        public function get_related_post(){
            $post_related_on = proactive()->get_theme_opt( 'post_related_on', false );
            ...
            if($post_related_on) {
                ...
                if (count($query_similar->posts) > 1) {
                    ...
                    <div class="pxl-related-post">
                        <h4 class="widget-title"><?php echo esc_html__('Related Posts', 'proactive'); ?></h4>
