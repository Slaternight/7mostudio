 ...
 * Google Fonts
*/
function proactive_fonts_url() {
    $fonts_url = '';
    $fonts     = array();
    $subsets   = 'latin,latin-ext';
    
    if ( 'off' !== _x( 'on', 'Barlow font: on or off', 'proactive' ) ) {
        $fonts[] = 'Barlow:ital,wght@0,400;0,500;0,600;0,700';
    }

    if ( 'off' !== _x( 'on', 'Fira Sans font: on or off', 'proactive' ) ) {
        $fonts[] = 'Fira Sans:ital,wght@0,400;0,500;0,600;0,700;0,800;0,900;1,400;1,500;1,600;1,700;1,800;1,900';
    }
 ...
 * Get page ID by Slug
*/
function proactive_get_id_by_slug($slug, $post_type){
 ...
 **/
function proactive_content_by_slug($slug, $post_type){
    $content = proactive_get_content_by_slug($slug, $post_type);

    $id = proactive_get_id_by_slug($slug, $post_type);
 ...
 * Get content by slug
 **/
function proactive_get_content_by_slug($slug, $post_type){
    $content = get_posts(
        array(
            'name'      => $slug,
            'post_type' => $post_type
 ...
 * Custom Comment List
 */
function proactive_comment_list( $comment, $args, $depth ) {
	if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
 ...
 * Paginate Links
 */
function proactive_ajax_paginate_links($link){
 ...
 * RGB Color
 */
function proactive_hex_rgb($color) {
 ...
 * Image Size Crop
 */
if(!function_exists('proactive_get_image_by_size')){
    function proactive_get_image_by_size( $params = array() ) {
 ...
 * Search Form
 */
function proactive_header_mobile_search_form() { 
    $search_mobile = proactive()->get_theme_opt( 'search_mobile', false );
    $search_placeholder_mobile = proactive()->get_theme_opt( 'search_placeholder_mobile' );
    if($search_mobile) : ?>
    <div class="pxl-header-mobile-search pxl-hide-xl">
        <form role="search" method="get" action="<?php echo esc_url(home_url( '/' )); ?>">
            <input type="text" placeholder="<?php if(!empty($search_placeholder_mobile)) { echo esc_attr($search_placeholder_mobile); } else { esc_attr_e('Search...', 'proactive'); } ?>" name="s" class="search-field" />
...
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_year_shortcode() {
        ob_start(); ?>
            <span><?php the_date('Y'); ?></span>
        <?php $output = ob_get_clean();
        return $output;
    }
    pxl_register_shortcode('pxl_year', 'proactive_year_shortcode');
}

/* Highlight Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_text_highlight_shortcode( $atts = array() ) {
    ...
    pxl_register_shortcode('highlight', 'proactive_text_highlight_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_image_highlight_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'id_image' => '',
        ), $atts));
    ...
    pxl_register_shortcode('highlight_image', 'proactive_image_highlight_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_text_highlight_shortcode_editor( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_highlight', 'proactive_text_highlight_shortcode_editor');
}

/* Typewriter Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_text_typewriter_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
        ), $atts));
    ...
    pxl_register_shortcode('typewriter', 'proactive_text_typewriter_shortcode');
}

/* Square Animate */
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_square_animate_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'columns' => '',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_square_animate', 'proactive_square_animate_shortcode');
}

/* Button Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_btn_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
         'style' => '',
         'icon_class' => '',
    ...
    pxl_register_shortcode('pxl_button', 'proactive_btn_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_btn_submit_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
    ...
    pxl_register_shortcode('pxl_button_submit', 'proactive_btn_submit_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_slider_arrow( $atts = array() ) {
        extract(shortcode_atts(array(
         'type' => 'next',
         'style' => 'style-1',
        ), $atts));
    ...
    pxl_register_shortcode('slider_arrow', 'proactive_slider_arrow');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_text_gradient_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
         'form' => '',
         'to' => '',
    ...
    pxl_register_shortcode('text_gradient', 'proactive_text_gradient_shortcode');
}

// Shortcode Row/Column Grid
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_start_row_shortcode( $atts = array() ) {
        ob_start(); ?>
    ...
    pxl_register_shortcode('pxl_start_row', 'proactive_start_row_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_end_row_shortcode( $atts = array() ) {
        ob_start(); ?>
            </div>
        </div>       
        <?php $output = ob_get_clean();
    ...
    pxl_register_shortcode('pxl_end_row', 'proactive_end_row_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_start_col_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'class' => 'col-12',
        ), $atts));
        ob_start(); ?>
    ...
    pxl_register_shortcode('pxl_start_column', 'proactive_start_col_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_end_col_shortcode( $atts = array() ) {
        ob_start(); ?>
        </div>  
        <?php $output = ob_get_clean();
        return $output;
    }
    pxl_register_shortcode('pxl_end_column', 'proactive_end_col_shortcode');
}
...
/* Gallery Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_gallery_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'link_video' => '',
         'images_id' => '',
         'col' => '2',
        ...
        foreach ($pxl_images as $key => $img_id) :
            ...
            <div class="pxl--item">
                <div class="pxl--item-inner">
                    <a href="<?php echo esc_url($thumbnail_url); ?>" data-elementor-lightbox-slideshow="pxl-gallery-<?php echo esc_attr($pxl_g_id); ?>"><?php echo proactive_html($thumbnail); ?></a>
    ...
    pxl_register_shortcode('pxl_gallery', 'proactive_gallery_shortcode');
}

/* Addd shortcode Video button */
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_video_button_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'link' => '',
         'text' => '',
         'class' => 'style1',
    ...
    pxl_register_shortcode('pxl_video_button', 'proactive_video_button_shortcode');
}

/* Get Category Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_post_category_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'items' => '6',
         'columns' => '2',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_post_category', 'proactive_post_category_shortcode');
}

/* Slider 1  */
if(function_exists( 'pxl_register_shortcode' )) {
    function proactive_slider_price_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'price' => '',
         'desc' => '',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_slider_price', 'proactive_slider_price_shortcode');
}
 ...
 */
add_filter('get_archives_link', 'proactive_wg_archive_count');
function proactive_wg_archive_count($links) {
    $dir = '';
 ...
 */
add_filter('wp_list_categories', 'proactive_wc_cat_count_span');
function proactive_wc_cat_count_span($links) {
    $dir = '';
 ...
 */
function proactive_get_mega_menu_builder_id(){
    $mn_id = [];
 ...
 */
function proactive_get_page_popup_builder_id(){
    $pp_id = [];
...
function proactive_mouse_move_animation() { 
    $mouse_move_animation = proactive()->get_theme_opt('mouse_move_animation', 'off'); 
    if($mouse_move_animation == 'on') {
        wp_enqueue_script( 'proactive-cursor', get_template_directory_uri() . '/assets/js/libs/cursor.js', array( 'jquery' ), '1.0.0', true ); ?>  
 ...
 * Start - Cookie Policy
 */
function proactive_cookie_policy() {
    $cookie_policy = proactive()->get_theme_opt('cookie_policy', 'hide');
    $cookie_policy_description = proactive()->get_theme_opt('cookie_policy_description');
    $cookie_policy_btntext = proactive()->get_theme_opt('cookie_policy_btntext');
    $cookie_policy_link = get_permalink(proactive()->get_theme_opt('cookie_policy_link')); 
 ...
 */
function proactive_subscribe_popup() {
    $subscribe = proactive()->get_theme_opt('subscribe', 'hide');
    $subscribe_layout = proactive()->get_theme_opt('subscribe_layout');
    $popup_effect = proactive()->get_theme_opt('popup_effect', 'fade');
    $args = [
 ...
 */
add_action( 'show_user_profile', 'proactive_user_fields' );
add_action( 'edit_user_profile', 'proactive_user_fields' );
function proactive_user_fields($user){
    ...
    ?>
    <h3><?php esc_html_e('Theme Custom', 'proactive'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="user_name"><?php esc_html_e('Author Name', 'proactive'); ?></label></th>
            <td>
        ...
        <tr>
            <th><label for="user_position"><?php esc_html_e('Author Position', 'proactive'); ?></label></th>
            <td>
...
add_action( 'personal_options_update', 'proactive_save_user_custom_fields' );
add_action( 'edit_user_profile_update', 'proactive_save_user_custom_fields' );
function proactive_save_user_custom_fields( $user_id )
{
...
function proactive_get_user_name() {
...
function proactive_get_user_position() {
...
/* Author Social */
function proactive_get_user_social() {
