...
if(!function_exists('proactive_get_post_grid')){
    function proactive_get_post_grid($posts = [], $settings = []){ 
        if (empty($posts) || !is_array($posts) || empty($settings) || !is_array($settings)) {
            return false;
        }
        switch ($settings['layout']) {
            case 'post-1':
                proactive_get_post_grid_layout1($posts, $settings);
                break;

            case 'portfolio-1':
                proactive_get_portfolio_grid_layout1($posts, $settings);
                break;

            case 'portfolio-2':
                proactive_get_portfolio_grid_layout2($posts, $settings);
                break;

            case 'service-1':
                proactive_get_service_grid_layout1($posts, $settings);
                break;
...
// Start Post Grid
//--------------------------------------------------
function proactive_get_post_grid_layout1($posts = [], $settings = []){ 
    extract($settings);
                    ...
                    <div class="pxl-post--meta">
                        <?php if($show_author == 'true'): ?>
                            <div class="pxl-post--author pxl-mr-10">
                                ...
                                <?php echo esc_html__('by', 'proactive'); ?>&nbsp;<?php echo esc_attr($current_user->user_login); ?>
                            </div>
                        <?php endif; ?>
                        <?php if($show_comment) : ?>
                        ...
                        <div class="pxl-post--button">
                            <a class="btn" href="<?php echo esc_url(get_permalink( $post->ID )); ?>">
                                <?php if(!empty($button_text)) {
                                    ...
                                    echo esc_html__('Read More', 'proactive');
                                } ?>
                            </a>
                        </div>
                    <?php endif; ?>
...
//--------------------------------------------------
function proactive_get_portfolio_grid_layout1($posts = [], $settings = []){ 
    extract($settings);
...
function proactive_get_portfolio_grid_layout2($posts = [], $settings = []){ 
    extract($settings);
            ...
            if (has_post_thumbnail($post->ID) && wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), false)): ?>
                <div class="<?php echo esc_attr($item_class . ' ' . $filter_class); ?>">
                    <div class="pxl-post--inner <?php echo esc_attr($pxl_animate); ?>" data-wow-duration="1.2s">
                        ...
                        $thumbnail    = $img['thumbnail'];
                        $gradient_from = proactive()->get_opt('gradient_color', ['from' => '#6000ff'])['from'];
                        $gradient_to = proactive()->get_opt('gradient_color', ['to' => '#fe0054'])['to'];
                        ?>
                        <div class="pxl-post--featured hover-imge-effect3">
                            <a href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php echo wp_kses_post($thumbnail); ?></a>
                        </div>
...
// Start Service Grid
//--------------------------------------------------
function proactive_get_service_grid_layout1($posts = [], $settings = []){ 
    extract($settings);
...
add_action( 'wp_ajax_proactive_load_more_post_grid', 'proactive_load_more_post_grid' );
add_action( 'wp_ajax_nopriv_proactive_load_more_post_grid', 'proactive_load_more_post_grid' );
function proactive_load_more_post_grid(){
    if ( ! check_ajax_referer( '_ajax_nonce', 'wpnonce' ) || empty( sanitize_text_field( wp_unslash($_POST['wpnonce'] ))) ) {
        wp_send_json(
            array(
                'status' => false,
                'message' => esc_attr__('Nonce error, please reload.', 'proactive'),
                'data' => array(),
            )
        );
    }
    ...
    try{
        if(!isset($_POST['settings'])){
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'proactive'));
        }
        ...
        ob_start();
            proactive_get_post_grid($posts, $settings);
        $html = ob_get_clean();

        $pagin_html = '';
        if( isset($settings['pagination_type']) && $settings['pagination_type'] == 'pagination' ){ 
            ob_start();
                proactive()->page->get_pagination( $query,  true );
            $pagin_html = ob_get_clean();
        }
        wp_send_json(
            array(
                'status' => true,
                'message' => esc_attr__('Load Successfully!', 'proactive'),
                'data' => array(
                    'html' => $html,
