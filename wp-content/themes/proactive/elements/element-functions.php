 ...
 * Swipper Lib
*/
if(!function_exists('proactive_elements_scripts')){
    add_action( 'wp_enqueue_scripts', 'proactive_elements_scripts');
    function proactive_elements_scripts() {  
        ...
        wp_register_script('proactive-particle', get_template_directory_uri() . '/elements/assets/js/particle.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('proactive-parallax', get_template_directory_uri() . '/elements/assets/js/parallax.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        ...
        wp_register_script('proactive-counter', get_template_directory_uri() . '/elements/assets/js/counter.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('proactive-accordion', get_template_directory_uri() . '/elements/assets/js/accordion.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('proactive-tabs', get_template_directory_uri() . '/elements/assets/js/tabs.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('proactive-progressbar', get_template_directory_uri() . '/elements/assets/js/progressbar.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        wp_register_script('proactive-countdown', get_template_directory_uri() . '/elements/assets/js/countdown.js', [ 'jquery' ], $theme->get( 'Version' ), true);
        ...
        wp_enqueue_script('proactive-elementor', get_template_directory_uri() . '/elements/assets/js/elementor.js', [ 'jquery' ], $theme->get( 'Version' ), true);
    }
}
...
*/
if(!function_exists('proactive_register_custom_icon_library')){
    add_filter('elementor/icons_manager/native', 'proactive_register_custom_icon_library');
    function proactive_register_custom_icon_library($tabs){
        $custom_tabs = [
            'pxl_icon1' => [
                'name' => 'flaticon',
                'label' => esc_html__( 'Proactive', 'proactive' ),
                'url' => false,
                'enqueue' => false,
                'prefix' => 'flaticon-',
...
*/
if(!function_exists('proactive_get_class_widget_path')){
    function proactive_get_class_widget_path(){
...
*/
function proactive_get_post_type_options($pt_supports=[]){
...
*/
function proactive_get_post_grid_layout($pt_supports = []){
    $post_types  = proactive_get_post_type_options($pt_supports); 
    $result = [];
    if (!is_array($post_types))
        return $result;
    foreach ($post_types as $name => $label) {
        $result[] = array(
            'name'     => 'layout_'.$name,
            'label'    => sprintf(esc_html__( 'Select Template of %s', 'proactive' ), $label),
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => proactive_get_grid_layout_options($name),
...
function proactive_get_grid_layout_options($post_type_name){
    $option_layouts = [];
...
function proactive_get_grid_term_by_post_type($pt_supports = [], $args=[]){
    ...
    foreach ($post_types as $name => $label) {
        ...
        $result[] = array(
            'name'     => 'source_'.$name,
            'label'    => sprintf(esc_html__( 'Select Term of %s', 'proactive' ), $label),
...
function proactive_get_grid_ids_by_post_type($pt_supports = [], $args = []){
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types = proactive_get_post_type_options($pt_supports);
    $result = [];
    if (!is_array($post_types))
    ...
    foreach ($post_types as $name => $label) {
        ...
        $posts = proactive_list_post($name, false);
 
        $result[] = array(
            'name' => 'source_' . $name . '_post_ids',
            'label' => sprintf(esc_html__('Select posts', 'proactive'), $label),
            'type'     => \Elementor\Controls_Manager::SELECT2,
            'multiple' => true,
            'options' => $posts,
            'condition' => array_merge(
...
*/
function proactive_get_post_carousel_layout($pt_supports = []){
    $post_types  = proactive_get_post_type_options($pt_supports); 
    $result = [];
    if (!is_array($post_types))
        return $result;
    foreach ($post_types as $name => $label) {
        $result[] = array(
            ...
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => proactive_get_carousel_layout_options($name),
...
function proactive_get_carousel_layout_options($post_type_name){
    $option_layouts = [];
    switch ($post_type_name) {
        ...
        case 'post':  
            $option_layouts = [
                ...
                'post-3' => [
                    'label' => esc_html__( 'Layout 3', 'proactive' ),
...
function proactive_get_carousel_term_by_post_type($pt_supports = [], $args=[]){
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types  = proactive_get_post_type_options($pt_supports); 
    $result = [];
    if (!is_array($post_types))
...
/* Icon render */ 
function proactive_elementor_icon_render( $settings, $args = []){
...
function proactive_widget_animate() {
    $proactive_animate = array(
        '' => 'None',
        'wow bounce' => 'bounce',
        'wow flash' => 'flash',
        'wow pulse' => 'pulse',
    ...
    return $proactive_animate;
}

function proactive_widget_animate_v2() {
    $proactive_animate_v2 = array(
        '' => 'None',
        'wow bounce' => 'bounce',
        'wow flash' => 'flash',
        'wow pulse' => 'pulse',
    ...
    return $proactive_animate_v2;
}

/**
 * Pagram Animation
*/
if(!function_exists('proactive_widget_animation_settings')){
    function proactive_widget_animation_settings($args = []){
        $args = wp_parse_args($args, [
        ...
        return array(
            ...
            'label'     => esc_html__('Animation', 'proactive'),
            'tab'       => $args['tab'],
...
if(!function_exists('proactive_widget_color_type')){
    function proactive_widget_color_type($args = []){
        ...
        $options = array(
            array(
                ...
                'label' => $args['label'] .' '.esc_html__('Color Type', 'proactive' ),
                'type' => \Elementor\Controls_Manager::SELECT,
                'options' => [
                    'normal' => 'Normal',
                    'gradient' => 'Gradient',
            ...
            array(
                ...
                'label' => $args['label'] .' '.esc_html__('Normal Color', 'proactive' ),
            ...
            array(
                ...
                'label' => $args['label'] .' '.esc_html__('Gradient Color', 'proactive' ),
            ...
            array(
                ...
                'label'       => ucfirst( str_replace('_', '', $args['prefix']) ).' '. esc_html__( 'Start Popover', 'proactive' ),
                'type'        => 'pxl_start_popover',
                'condition'   => $args['condition'],
            ),
            array(
                'name' => $args['prefix'].'_gradient_color_from',
                'label' => esc_html__('From', 'proactive' ),
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
            ...
            array(
                ...
                'label'       => ucfirst( str_replace('_', '', $args['prefix']) ).' '. esc_html__( 'End Popover', 'proactive' ),
...
if(!function_exists('proactive_widget_bgcolor_type')){
    function proactive_widget_bgcolor_type($args = []){
        $gradient_prefix_class = 'pxl-';
        $gradient_return_value = 'gradient';
        $args = wp_parse_args($args, [
            'label' => '',
        ...
        $options = array(
            array(
                'name' => $args['prefix'] .'_color_type',
                'label' => esc_html__('BG Color Type', 'proactive' ),
                'type' => \Elementor\Controls_Manager::SELECT,
                'options' => [
                    'normal' => 'Normal',
                    'gradient' => 'Gradient 1',
            ...
            array(
                'name' => $args['prefix'] .'_normal_color',
                'label' => esc_html__('Box Normal Color', 'proactive' ),
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
            ...
            array(
                'name'        => $args['prefix'].'_gradient_color',
                'label' => $args['label'] .' '.esc_html__('BG Gradient Color', 'proactive' ),
                'type' => \Elementor\Controls_Manager::POPOVER_TOGGLE,
                'prefix_class' => $gradient_prefix_class,
                'return_value' => $gradient_return_value,
                'condition' => [
...
if(!function_exists('proactive_widget_gradient_color')){
    function proactive_widget_gradient_color($args = []){
        $gradient_prefix_class = 'pxl-';
        $gradient_return_value = 'gradient';
        $args = wp_parse_args($args, [
            'label' => '',
...
if(!function_exists('proactive_widget_gradient_color_rotate')){
    function proactive_widget_gradient_color_rotate($args = []){
        $gradient_prefix_class = 'pxl-';
        $gradient_return_value = 'gradient';
        $args = wp_parse_args($args, [
            'label' => '',
        ...
        $options = array(
            ...
            array(
                ...
                'label' => esc_html__('Angle', 'proactive' ),
